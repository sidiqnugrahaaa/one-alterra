...
@section('content')
    @if ($currentContent->content->type == 5)
        <form>
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="{{ $currentContent->status == 0 ? 'dropzone-file' : '' }}"
                            ...
                            <div class="flex flex-col items-center justify-center pt-5 pb-6 file-info">
                                @if ($currentContent->status == 1)
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-8 h-8 mb-4 text-gray-500">
                            ...
                            @if ($currentContent->status == 0)
                                <input id="dropzone-file" type="file" name="file_task" class="hidden"
                                    onchange="changefile()" />
                            @endif
                        </label>
                    </div>
